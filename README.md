...
**Task Queue (tq)** 旨在解决实验室服务器资源分配的核心痛点：**如何在闲时充分利用 GPU 跑长任务，同时在需要调试时立即获得 GPU 使用权，且不丢失后台任务进度？**

它允许你将大量低优先级的训练任务推入后台队列。一旦检测到高优先级活动（如启动 Jupyter 进行调试或手动运行验证脚本），`tq` 会自动暂停或安全终止后台任务，释放显卡资源。待资源空闲后，调度器会自动恢复之前的任务（支持断点续训）。
...
在单机科研场景下，我们通常面临两难选择：要么手动管理极其繁琐，要么配置企业级调度器过于复杂。`tq` 旨在填补这一空白。

| 工具 | 类型 | 适用场景 | tq 与之相比的优势 |
| :--- | :--- | :--- | :--- |
| **tmux / screen** | 会话管理 | 手动跑实验，需要保持终端不掉线。 | **自动排队 & 抢占**。在 tmux 中，你必须手动等前一个跑完再敲下一个；想调试时必须手动 Ctrl+C 杀掉后台任务再重启新任务；想要在新任务跑完恢复旧的长期训练任务需要等当前任务执行完重新敲指令重启训练，非常繁琐。`tq` 自动化了这一切。 |
| **nohup / &** | 后台运行 | 简单的“发后即忘”任务。 | **有序调度**。用 `&` 同时丢 5 个训练任务会导致 GPU 显存爆炸 (OOM)；无法实现队列执行和抢占恢复机制。`tq` 保证任务串行执行，可以分配多个任务队列且自动记录日志，进行优先级高的临时调试结束后自动恢复低优先级的长期训练任务。 |
| **Slurm / K8s** | 集群调度 | 多节点、多用户、需要计费和严格权限控制的超算中心。 | **极致轻量 & 调试友好**。Slurm 需要管理员权限配置数据库和守护进程，且通常难以实现“我一打开 Jupyter，后台任务就自动暂停”的灵活性。`tq` 零配置，开箱即用。 |
| **CUDA MPS** | 驱动特性 | 多进程共享同一 GPU 显存和算力。 | **完全不同的层级**。MPS 解决的是“如何让两个任务挤在一张卡上”；`tq` 解决的是“谁先跑，谁后跑”。`tq` 通过排队避免了显存争抢，通常比 MPS 共享更适合显存密集的深度学习训练。 |
